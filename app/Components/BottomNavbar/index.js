...
export default function BottomNavbar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t-[3px] border-[#265BA7] rounded-t-2xl px-2 py-2 flex justify-center items-center text-sm sm:hidden z-50">
      {navItems.map((item, index) => (
        <div key={item.key} className="flex items-center">
          ...
          >
            <div
              onClick={() => setActiveTab(item.key)}
              className={`flex flex-col items-center px-2 py-2 rounded-[12px] cursor-pointer ${
                activeTab === item.key
